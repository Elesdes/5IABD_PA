                ...
                <tr class="text-gray-700 dark:text-gray-400" id="tabletr${model.idModel}">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center space-x-4 text-sm">
                            <button class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray" aria-label="Download" onclick="downloadMyModel('${model.idModel}')">
...
function downloadMyModel(idModel) {
    fetch(`/api/download_mymodels/?idModel=${idModel}`, {
        method: 'GET'
    })
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        console.log('Modèle téléchargé avec succès:', data);
    })
    .catch(error => {
        console.error('Une erreur s\'est produite lors du téléchargement du modèle:', error);
    }).finally(() => {
        window.location.reload();
    });
}
