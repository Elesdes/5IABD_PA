...
from fastapi import Request, HTTPException, APIRouter, status, Depends, Form, Cookie, Response
...
def get_user(cursor, email=None, cookie=None):
    ...
    print(SQL_query)
    if user_data:
        return User(user_data['email'], user_data['password'], user_data['name'], user_data['forename'],  user_data['role'], user_data['cookie'])
    return None


def insert_user(connector, cursor, email, password, name, forename):
    cursor.execute(f"INSERT INTO users(email, password, name, forename, role, cookie) VALUES('{email}', '{password}', '{name}', '{forename}', 2, '')")
    connector.commit()
...
async def register(email: Annotated[str, Form()], password: Annotated[str, Form()], name: Annotated[str, Form()], forename: Annotated[str, Form()], response: Response, request: Request):
    DB = ConfigDB()
    cursor = DB.get_db_cursor()
    ...
    user = insert_user(DB.connector, cursor, email, pwd_context.hash(password, scheme="md5_crypt"), name, forename)
    ...
    response.set_cookie(key="ICARUS-Login", value=f"{user.cookie}", expires=expiry, secure=True, samesite="none")
    return templates.TemplateResponse(
        name="index.html", context={"request": request}
    )
...
@router.post("/login")
async def login(email: Annotated[str, Form()], password: Annotated[str, Form()], response: Response, request: Request):
    DB = ConfigDB()
    cursor = DB.get_db_cursor()
    user = get_user(cursor, email)
    if not user or not user.verify_password(password):
    ...
    DB.connector.close()
    expiry = datetime.datetime.now(datetime.timezone.utc) + timedelta(days=1)
    response.set_cookie(key="ICARUS-Login", value=f"{cookie_value}", expires=expiry, secure=True, samesite="none")
    return templates.TemplateResponse(
        name="dashboard.html", context={"request": request}
    )
